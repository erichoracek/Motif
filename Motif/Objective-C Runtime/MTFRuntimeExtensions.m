...
mtf_propertyAttributes *mtf_copyPropertyAttributes (objc_property_t property) {
    ...
    mtf_propertyAttributes *attributes = calloc(1, sizeof(mtf_propertyAttributes) + typeLength + 1);
    if (!attributes) {
        fprintf(stderr, "ERROR: Could not allocate mtf_propertyAttributes structure for attribute string \"%s\" for property %s\n", attrString, property_getName(property));
        return NULL;
    }
    ...
    while (next && *next == ',') {
        ...
        case 'C':
            attributes->memoryManagementPolicy = mtf_propertyMemoryManagementPolicyCopy;
            break;

        case '&':
            attributes->memoryManagementPolicy = mtf_propertyMemoryManagementPolicyRetain;
            break;
