...
+ (void)load {
    ...
    [self
        ...
        returningTransformedValueWithBlock:^(NSArray *values) {
            NSAssert(values.count <= 4, @"Values array must have at most four values");
            NSAssert(values.count > 1, @"Values array must have more than one value");
            ...
            typeof(TransformedValueCType) value;
            if (values.count == 2) {
                value = (typeof(TransformedValueCType)){
