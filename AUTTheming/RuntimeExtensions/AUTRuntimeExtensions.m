...
aut_propertyAttributes *aut_copyPropertyAttributes (objc_property_t property) {
    ...
    aut_propertyAttributes *attributes = calloc(1, sizeof(aut_propertyAttributes) + typeLength + 1);
    if (!attributes) {
        fprintf(stderr, "ERROR: Could not allocate aut_propertyAttributes structure for attribute string \"%s\" for property %s\n", attrString, property_getName(property));
        return NULL;
    }
    ...
    while (next && *next == ',') {
        ...
        case 'C':
            attributes->memoryManagementPolicy = aut_propertyMemoryManagementPolicyCopy;
            break;

        case '&':
            attributes->memoryManagementPolicy = aut_propertyMemoryManagementPolicyRetain;
            break;
