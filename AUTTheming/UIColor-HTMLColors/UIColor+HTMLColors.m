...
static inline AUTFloatTriple HSB2HSL(CGFloat hue, CGFloat saturation, CGFloat brightness);
static inline AUTFloatTriple HSL2HSB(CGFloat hue, CGFloat saturation, CGFloat lightness);

static NSArray *AUTW3CColorNames(void);
static NSDictionary *AUTW3CNamedColors(void);
...
+ (UIColor *)aut_colorWithCSS:(NSString *)cssColor
{
    ...
    [scanner aut_scanCSSColor:&color];
...
+ (UIColor *)aut_colorWithHexString:(NSString *)hexColor
{
    ...
    [scanner aut_scanHexColor:&color];
    return (scanner.isAtEnd) ? color : nil;
}
...
+ (UIColor *)aut_colorWithRGBString:(NSString *)rgbColor
{
    ...
    [scanner aut_scanRGBColor:&color];
    return (scanner.isAtEnd) ? color : nil;
}
...
+ (UIColor *)aut_colorWithHSLString:(NSString *)hslColor
{
    ...
    [scanner aut_scanHSLColor:&color];
    return (scanner.isAtEnd) ? color : nil;
}
...
+ (UIColor *)aut_colorWithW3CNamedColor:(NSString *)namedColor
{
    ...
    [scanner aut_scanW3CNamedColor:&color];
    return (scanner.isAtEnd) ? color : nil;
}
...
- (NSString *)aut_hexStringValue
{
    ...
    if ([self aut_getRed:&red green:&green blue:&blue alpha:&alpha]) {
...
- (NSString *)aut_hslStringValue
{
    ...
    if ([self aut_getHue:&hue saturation:&saturation brightness:&brightness alpha:&alpha]) {
        AUTFloatTriple hslVal = HSB2HSL(hue, saturation, brightness);
        if (alpha == 1.0) {
...
- (BOOL)aut_getRed:(CGFloat *)red green:(CGFloat *)green blue:(CGFloat *)blue alpha:(CGFloat *)alpha
{
...
- (BOOL)aut_getHue:(CGFloat *)hue saturation:(CGFloat *)saturation brightness:(CGFloat *)brightness alpha:(CGFloat *)alpha
{
...
{
    return [[AUTW3CNamedColors() allKeys] sortedArrayUsingSelector:@selector(compare:)];
}
...
{
    return [self aut_scanHexColor:color]
        || [self aut_scanRGBColor:color]
        || [self aut_scanHSLColor:color]
        || [self aut_scanTransparent:color]
        || [self aut_scanW3CNamedColor:color];
}

- (BOOL)aut_scanRGBColor:(UIColor **)color
{
    return [self aut_caseInsensitiveWithCleanup:^BOOL{
        if ([self scanString:@"rgba" intoString:NULL]) {
            AUTFloatQuad scale = {1.0f/255.0f, 1.0f/255.0f, 1.0f/255.0f, 1.0f};
            AUTFloatQuad q;
            if ([self aut_scanFloatQuad:&q scale:scale]) {
                if (color) {
            ...
            AUTFloatTriple scale = {1.0f/255.0f, 1.0f/255.0f, 1.0f/255.0f};
            AUTFloatTriple t;
            if ([self aut_scanFloatTriple:&t scale:scale]) {
                if (color) {
...
static inline CGFloat AUTNormHue(CGFloat hue)
{
...
- (BOOL)aut_scanHSLColor:(UIColor **)color
{
    return [self aut_caseInsensitiveWithCleanup:^BOOL{
        if ([self scanString:@"hsla" intoString:NULL]) {
            AUTFloatQuad scale = {1.0f/360.0f, 1.0f, 1.0f, 1.0f};
            AUTFloatQuad q;
            if ([self aut_scanFloatQuad:&q scale:scale]) {
                if (color) {
                    AUTFloatTriple t = HSL2HSB(AUTNormHue(q.a), q.b, q.c);
                    *color = [UIColor colorWithHue:t.a saturation:t.b brightness:t.c alpha:q.d];
                }
                return YES;
            }
        } else if ([self scanString:@"hsl" intoString:NULL]) {
            AUTFloatTriple scale = {1.0f/360.0f, 1.0f, 1.0f};
            AUTFloatTriple t;
            if ([self aut_scanFloatTriple:&t scale:scale]) {
                if (color) {
                    t = HSL2HSB(AUTNormHue(t.a), t.b, t.c);
...
{
    return [self aut_resetScanLocationOnFailure:^BOOL{
        return [self scanString:@"#" intoString:NULL]
            && [self aut_scanHexTriple:color];
    }];
}
...
{
    return [self aut_caseInsensitiveWithCleanup:^BOOL{
        NSArray *colorNames = AUTW3CColorNames();
        NSDictionary *namedColors = AUTW3CNamedColors();
        for (NSString *name in colorNames) {
            if ([self scanString:name intoString:NULL]) {
                if (color) {
                    *color = [UIColor aut_colorWithHexString:namedColors[name]];
                }
                return YES;
            }
        }
...
- (void)aut_withSkip:(NSCharacterSet *)chars run:(void (^)(void))block
{
...
- (void)aut_withNoSkip:(void (^)(void))block
{
    NSCharacterSet *skipped = self.charactersToBeSkipped;
...
- (NSRange)aut_rangeFromScanLocation
{
...
- (void)aut_skipCharactersInSet:(NSCharacterSet *)chars
{
    [self aut_withNoSkip:^{
...
- (void)aut_skip
{
    [self aut_skipCharactersInSet:self.charactersToBeSkipped];
}

- (BOOL)aut_resetScanLocationOnFailure:(BOOL (^)(void))block
{
...
- (BOOL)aut_caseInsensitiveWithCleanup:(BOOL (^)(void))block
{
    NSUInteger initialScanLocation = self.scanLocation;
...
- (NSRange)aut_scanCharactersInSet:(NSCharacterSet *)chars maxLength:(NSUInteger)maxLength intoString:(NSString **)outString
{
    NSRange range = [self aut_rangeFromScanLocation];
...
// Hex characters
static NSCharacterSet *AUTHexCharacters() {
...
static NSUInteger AUTParseHex(NSString *str, BOOL repeated)
{
    unsigned int ans = 0;
    if (repeated) {
...
- (BOOL)aut_scanHexTriple:(UIColor **)color
{
    NSString *hex = nil;
    NSRange range = [self aut_scanCharactersInSet:AUTHexCharacters() maxLength:6 intoString:&hex];
    CGFloat red, green, blue;
    if (hex.length == 6) {
        // Parse 2 chars per component
        red   = AUTParseHex([hex substringWithRange:NSMakeRange(0, 2)], NO) / 255.0f;
        green = AUTParseHex([hex substringWithRange:NSMakeRange(2, 2)], NO) / 255.0f;
        blue  = AUTParseHex([hex substringWithRange:NSMakeRange(4, 2)], NO) / 255.0f;
    } else if (hex.length >= 3) {
        // Parse 1 char per component, but repeat it to calculate hex value
        red   = AUTParseHex([hex substringWithRange:NSMakeRange(0, 1)], YES) / 255.0f;
        green = AUTParseHex([hex substringWithRange:NSMakeRange(1, 1)], YES) / 255.0f;
        blue  = AUTParseHex([hex substringWithRange:NSMakeRange(2, 1)], YES) / 255.0f;
...
// Scan "transparent"
- (BOOL)aut_scanTransparent:(UIColor **)color
{
    return [self aut_caseInsensitiveWithCleanup:^BOOL{
...
// percentage.
- (BOOL)aut_scanNum:(CGFloat *)value scale:(CGFloat)scale
{
    float f = 0.0;
    if ([self scanFloat:&f]) {
...
- (BOOL)aut_scanFloatTriple:(AUTFloatTriple *)triple scale:(AUTFloatTriple)scale
{
    __block BOOL success = NO;
    __block AUTFloatTriple t;
    [self aut_withSkip:[NSCharacterSet whitespaceAndNewlineCharacterSet] run:^{
        success = [self scanString:@"(" intoString:NULL]
            && [self aut_scanNum:&(t.a) scale:scale.a]
            && [self scanString:@"," intoString:NULL]
            && [self aut_scanNum:&(t.b) scale:scale.b]
            && [self scanString:@"," intoString:NULL]
            && [self aut_scanNum:&(t.c) scale:scale.c]
            && [self scanString:@")" intoString:NULL];
    }];
    if (triple) {
        *triple = t;
...
- (BOOL)aut_scanFloatQuad:(AUTFloatQuad *)quad scale:(AUTFloatQuad)scale
{
    __block BOOL success = NO;
    __block AUTFloatQuad q;
    [self aut_withSkip:[NSCharacterSet whitespaceAndNewlineCharacterSet] run:^{
        success = [self scanString:@"(" intoString:NULL]
            && [self aut_scanNum:&(q.a) scale:scale.a]
            && [self scanString:@"," intoString:NULL]
            && [self aut_scanNum:&(q.b) scale:scale.b]
            && [self scanString:@"," intoString:NULL]
            && [self aut_scanNum:&(q.c) scale:scale.c]
            && [self scanString:@"," intoString:NULL]
            && [self aut_scanNum:&(q.d) scale:scale.d]
            && [self scanString:@")" intoString:NULL];
    }];
    if (quad) {
        *quad = q;
...
static inline AUTFloatTriple HSL2HSB(CGFloat hue, CGFloat saturation, CGFloat l)
{
    l *= 2.0f;
...
static NSArray *AUTW3CColorNames() {
    static NSArray *colorNames;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        colorNames = [[AUTW3CNamedColors() allKeys] sortedArrayUsingComparator:^NSComparisonResult(NSString *k1, NSString *k2) {
...
static NSDictionary *AUTW3CNamedColors() {
